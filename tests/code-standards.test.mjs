...
for (const file of sourceFiles(root)) {
  ...
  assert.ok(lines.length <= 200, `${repoPath} exceeds the 200-line limit`)
  for (const [index, line] of lines.entries()) {
    const length = [...line].length
    assert.ok(length <= 200, `${repoPath}:${index + 1} exceeds the 200-character limit`)
  }
}

console.log('code standards passed')
