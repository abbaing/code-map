...
for (const [file, imports] of dependencies) {
  assert.equal(imports.some(target => target.includes(`${path.sep}tests${path.sep}`)), false, `${relative(file)} must not import tests`)
}

const coreFiles = productionFiles.filter(file => {
  const name = relative(file)
  return name === 'graph.mjs'
...
for (const file of coreFiles) {
  ...
  assert.equal(
    imports.some(target => target === 'cli.mjs' || target === 'server.mjs' || target === 'server-app.mjs' || target.startsWith('viewer/')),
    false,
...
assert.deepEqual(importSpecifiers(graphSource), [], 'Graph must remain independent from configuration, filesystems, CLI, and HTTP')
...
assert.doesNotMatch(fs.readFileSync(path.join(root, 'server-app.mjs'), 'utf8'), /node:http/u, 'application use cases must not depend on HTTP')
assert.deepEqual(
...
function listSourceFiles(directory) {
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (/\.(?:mjs|js)$/u.test(entry.name)) files.push(target)
  }
  return files.sort()
}
