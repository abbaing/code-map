...
function initializeFilters() {
  ...
  els.uncoveredCount.textContent = String(
    nodes.filter((node) => isCoverable(node) && !node.meta?.coverage?.hasCoverage).length
  )
  ...
  const findingScopes = findings.reduce((acc, finding) => {
    const scope = finding.ruleId?.startsWith('repo.')
      ? 'repo'
      : finding.ruleId?.startsWith('framework.')
        ? 'framework'
        : finding.ruleId?.startsWith('technology.')
          ? 'technology'
          : 'other'
...
function buildFilterPredicate() {
  ...
  const predicates = [
    ...
    (node) =>
      isOverview ||
      !query ||
      [node.label, node.path, node.id, node.module, node.layer, node.type, formatType(node.type)]
        .filter(Boolean)
