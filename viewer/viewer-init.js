...
function bindPanDrag(canvas) {
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    if (event.target.closest('#moduleDetail')) {
      return
    }
...
els.overviewScroll.addEventListener('click', (event) => {
  ...
  if (card) {
    drillIntoModule(card.dataset.module)
  }
})
...
els.tabOverview.addEventListener('click', () => {
  state.view = 'overview'
  state.activeModule = null
  updateViewUI()
  applyFilters()
...
els.tabDomain.addEventListener('click', () => {
  state.view = 'domain'
  state.activeModule = null
  state.panX = 0
  state.panY = 0
...
els.tabFindings.addEventListener('click', () => {
  state.view = 'findings'
  state.activeModule = null
  updateViewUI()
  applyFilters()
