...
export function normalizeRequest(request = {}) {
  ...
  if (!/^[a-z0-9][a-z0-9._-]*$/i.test(id)) {
    throw new SubmapError('SUBMAP_INVALID_ID', 'Submap id must use letters, numbers, dots, underscores, or hyphens.', { id })
  }

  const direction = request.traversal?.direction ?? 'both'
  if (!['incoming', 'outgoing', 'both'].includes(direction)) {
    throw new SubmapError('SUBMAP_INVALID_DIRECTION', 'Traversal direction must be incoming, outgoing, or both.', { direction })
  }
  ...
  if (request.parentUid != null && !/^sha256:[a-f0-9]{64}$/.test(request.parentUid)) {
    throw new SubmapError('SUBMAP_INVALID_PARENT_UID', 'parentUid must be a SHA-256 identifier.', { parentUid: request.parentUid })
  }
...
export function resolveSeeds(graph, selectors) {
  ...
  if (missingNodeIds.length) {
    throw new SubmapError('SUBMAP_SEED_NOT_FOUND', 'One or more explicit seed nodes were not found.', { nodeIds: missingNodeIds }, 3)
  }

  const ids = new Set(selectors.nodeIds)
  for (const node of graph.nodes) {
    if (selectors.paths.some(pattern => globMatches(pattern, node.path ?? ''))) ids.add(node.id)
    if (matchesAttributeQuery(node, selectors)) ids.add(node.id)
  }
...
export function resolveSelectorNodeIds(nodes, selector) {
  ...
  for (const node of nodes) {
    if (selector.paths.some(pattern => globMatches(pattern, node.path ?? ''))) ids.add(node.id)
    if (matchesAttributeQuery(node, selector)) ids.add(node.id)
  }
  return ids
}
...
function matchesAttributeQuery(node, selector) {
  ...
  return (!selector.modules.length || selector.modules.includes(node.module))
...
function strings(values = []) {
  ...
  return [...new Set(list.filter(value => value !== undefined && value !== null).map(value => String(value).trim()).filter(Boolean))].sort()
}
...
function validateRequestShape(request) {
  if (!isObject(request)) throw new SubmapError('SUBMAP_INVALID_REQUEST', 'Submap request must be a JSON object.')
  assertKnownKeys(request, ['id', 'revision', 'parentUid', 'selectors', 'traversal', 'exclusions', 'access', 'metadata'], 'request')
  ...
  if (request.traversal !== undefined) {
    if (!isObject(request.traversal)) throw new SubmapError('SUBMAP_INVALID_REQUEST', 'traversal must be an object.')
  ...
  if (request.access !== undefined) {
    if (!isObject(request.access)) throw new SubmapError('SUBMAP_INVALID_REQUEST', 'access must be an object.')
    assertKnownKeys(request.access, ['default', ...ACCESS_LEVELS], 'access')
    for (const level of ACCESS_LEVELS) validateSelectorShape(request.access[level], `access.${level}`)
  }
  if (request.metadata !== undefined && !isObject(request.metadata)) throw new SubmapError('SUBMAP_INVALID_REQUEST', 'metadata must be an object.')
}

function validateSelectorShape(selector, location) {
  if (selector === undefined) return
  if (!isObject(selector)) throw new SubmapError('SUBMAP_INVALID_REQUEST', `${location} must be an object.`)
  assertKnownKeys(selector, ['nodeIds', 'nodes', 'paths', 'modules', 'layers', 'types'], location)
  for (const key of ['nodeIds', 'nodes', 'paths', 'modules', 'layers', 'types']) validateStringArray(selector[key], `${location}.${key}`)
}
...
function assertKnownKeys(value, allowed, location) {
  ...
  if (unknown.length) throw new SubmapError('SUBMAP_UNKNOWN_REQUEST_PROPERTY', `Unknown properties in ${location}.`, { location, properties: unknown.sort() })
}
